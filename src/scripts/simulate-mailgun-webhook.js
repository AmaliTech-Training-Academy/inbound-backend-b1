...
const rawMime = INCLUDE_ATTACHMENT
  ? [
      ...
      ``,
      `--${boundary}`,
      `Content-Type: text/plain; name="attachment_second_test.txt"`,
      `Content-Disposition: attachment; filename="attachment_second_test.txt"`,
      `Content-Transfer-Encoding: base64`,
      ``,
      Buffer.from("Sending a second attachment for testing.").toString("base64"),
      ``,
      `--${boundary}--`,
    ].join("\r\n")
  : [
